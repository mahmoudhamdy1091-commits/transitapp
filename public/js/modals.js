...
export async function submitCollection() {
  ...
  // ✅ تاريخ الدفع إجباري دايمًا في هذه الشاشة — "تحصيل من عميل" فعل تسجيل
  // استلام فعلي، مش مجرد تسجيل مبلغ مستحق. من غير تاريخ دفع، السطر يتسجّل
  // "مستحق" بلا أي قيد محاسبي بصمت رغم تحديد مستلم — اتكشف حيًّا 2026-07-28
  // مرتين (BOX-133، LOT 3 NEW): المستخدم حدد "استلمه: الصندوق" وحفظ من غير
  // تاريخ دفع، فطلع تحصيل مكرر "مستحق" بدل ما يتسجل كمدفوع فعليًا.
  if (!paid) {
    showFieldErr('colError', '⚠️ يجب تحديد تاريخ الدفع — هذه الشاشة لتسجيل تحصيل فعلي، مش مبلغ مستحق فقط');
    return;
  }
...
// ✅ wrapper رفيع فوق computePartnerSettlement (core.js) — المصدر الموحّد
// نفسه المستخدم في تبويب الملخص وكشف حساب الشريك، فيضمن نفس رقم "المستحق"
// في كل شاشة (كان محسوبًا هنا بمعادلة منفصلة ناقصة مصاريف الجيب والتحصيلات).
// شكل الإرجاع مطابق للقديم حتى لا يحتاج المستدعي (onPayoutPartnerChange) أي تعديل.
