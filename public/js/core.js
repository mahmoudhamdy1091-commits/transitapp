...
export async function computePartnerSettlement(fileNo, sys) {
  ...
  const partners = (partnersRaw||[]).map(p => {
    ...
    //     نهائي فأي سقف مبني عليه هيبقى تعسفيًا. ⚠️ فرع مفتوح فيه شبهة معاكسة
    //     (تساهل لا تقييد — شريك بمساهمة صفر يقدر يسحب من نقد شريك ساهم بالكامل،
    //     مُثبَت حيًّا 2026-09-06) قيد المراجعة، لم يُعدَّل بعد بانتظار قرار صريح.
    // الوعد باسترداد رأس مال لسه محبوس في مخزون غير مباع هو وعد بفلوس مش موجودة
    const payableNow = isClosed
      ? Math.max(0, actualContribution + profitShare - withdrawnViaPayout)
