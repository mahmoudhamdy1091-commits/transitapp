...
export async function submitQuickPayout() {
  ...
  try {
    ...
    showFieldErr('qsPoError', 'تعذّر التحقق من المستحق قبل الصرف — لم يُحفظ شيء. حاول مرة أخرى (' + e.message + ')');
    return;
  }
  if (!capChk.ok) { showFieldErr('qsPoError', capChk.message); return; }
  if (capChk.warning) {
