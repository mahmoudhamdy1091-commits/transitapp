-- نفّذ هذا في Supabase SQL Editor مرة واحدة. ✅ نُفِّذ بالكامل 2026-09-16 (الأقسام 1-7)،
-- ومعه القسم 9 (RLS + policy القراءة) بعد اكتشاف أن CREATE TABLE فعّل RLS بصفر policy.
-- مُتحقَّق حيًّا بعده: 9 صفوف ربط + 9 حسابات شركاء في الشجرة (8 BOX + 1 TM).
--
...
--
-- ⚠️ لا تُعِد تشغيل هذا الملف بعد م١ (docs/PLAN-partner-accounts-2026-09-17.md):
-- القسم 6 تحته فيه أسماء قديمة بالحرف ('ابو هادي', 'أبو أسعد') اتغيّرت فعليًا في
-- partner_account_links (راجع sql/m1_rename_abu_hadi_and_abu_assad.sql). الـ
-- ON CONFLICT هنا على (system_type, partner_name) لا account_code، فإعادة
-- التشغيل لن ترفض — هتضيف صف تاني ('BOX','ابو هادي','2401') جنب الصف الحالي
-- ('BOX','عبد الرحيم الجاحد','2401')، فيرجع نفس فخ الاسمين لنفس الحساب اللي
-- rename_partner اتعمل أصلًا عشان يمنعه (اكتشاف مراجعة خارجية، 2026-09-17).
